...
void serialKom (){
  ...
  Serial.println(aktFeuchtigkeit);
  Serial.println("Helligkeit: ");
  Serial.println(aktHelligkeit);
  
  //auszugeben und zu verändern
  Serial.println("min_Bodenfeuchte: ");
  Serial.println(minFeuchtigkeit );
  Serial.println("min_Helligkeit: ");
  Serial.println(minHelligkeit );
  Serial.println("GiesInterval: ");
  Serial.println(giesInterval );
  Serial.println("MessInterval: ");
  Serial.println(messInterval );
  
  Serial.flush(); 
  
  // 1: min_Bodenfeuchte aendern, 2: min_Helligkeit aendern, 3: GiesInterval aendern, 4: MessInterval aendern
  if(Serial.available()){
    
   menueauswahl = Serial.read(); //geplant ist ein Menue im (C++)Programm, bei dem man erstmal sagt, was geaendert werden soll. Im naechsten Schritt wird der neue Wert eingegeben
    
    
    switch(menueauswahl){             // geplant ist, fehlerhafte Eingaben im Programm zu behandeln
      case 1: 
            if(Serial.available()){
              minFeuchtigkeit = Serial.read();
            }
      break;
      case 2:
            if(Serial.available()){
              minHelligkeit  = Serial.read();
            }
      break;
      case 3:
            if(Serial.available()){
              giesInterval = Serial.read();
            }
      break;
      case 4:
           if(Serial.available()){
              messInterval = Serial.read();
            }
      break;
    }
  menueauswahl = 0;
