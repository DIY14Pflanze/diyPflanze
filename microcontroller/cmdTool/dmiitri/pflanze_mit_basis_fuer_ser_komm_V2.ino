...
void printLCD(int mode) {
  ...
  // 1: min_Bodenfeuchte aendern, 2: min_Helligkeit aendern, 3: GiesInterval aendern, 4: MessInterval aendern, 5-10: Jeweilige Variable ausgeben,
  if(Serial.available()){
    
   menueauswahl = (Serial.read() - '0'); //geplant ist ein Menue im (C++)Programm, bei dem man erstmal sagt, was geaendert werden soll. Im naechsten Schritt wird der neue Wert eingegeben
    
    
    switch(menueauswahl){             // geplant ist, fehlerhafte Eingaben im Programm zu behandeln
      case 1: 
            if(Serial.available()){
              min_Bodenfeuchte = Serial.read();
            }
      break;
      case 2:
            if(Serial.available()){
              min_Helligkeit = Serial.read();
            }
      break;
      case 3:
            if(Serial.available()){
      ...
      break;
      case 5:
           Serial.print(aktFeuchtigkeit);
      break;
      case 6:
          Serial.print(aktHelligkeit);
      break;
      case 7:
          Serial.print(min_Bodenfeuchte);
      break;
      case 8:
          Serial.print(min_Helligkeit);
      break;
      case 9:
          Serial.print(GiesInterval);
      break;
      case 10:
